...
COMMENT * Коды некоторых исключений
  C0000005h – Поток пытался считать или записать в память, не имея на то необходимых прав
  C000001Dh – Поток пытался передать управление в страницы сегмента данных
  C000008Ch – Поток пытался использовать значение регистра за границами BOUND
  C0000094h – Поток пытался поделить число целого типа на делитель того же типа, равный 0
  C0000095h – Поток пытался поделить с целочисленным переполнением (ax=20000 div bl=2)
  C00000FDh – Стек, отведенный потоку, исчерпан
  80000001h – Поток пытался обратиться к странице памяти с атрибутом защиты PAGE_GUARD
